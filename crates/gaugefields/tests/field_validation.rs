...
fn link_validation_rejects_dtype_rank_and_shape() {
    ...
    assert!(matches!(
        GaugeLinkTensor::try_from_tensor(wrong_dtype, lattice),
        Err(GaugeError::DType { .. })
    ));
    let wrong_rank =
        Tensor::from_vec_col_major(vec![3, 3, 16], vec![Complex64::default(); 144]).unwrap();
    assert!(matches!(
        GaugeLinkTensor::try_from_tensor(wrong_rank, lattice),
        Err(GaugeError::Rank { .. })
    ));
    let wrong_shape =
        Tensor::from_vec_col_major(vec![3, 3, 2, 2, 2, 3], vec![Complex64::default(); 216])
            .unwrap();
    assert!(matches!(
        GaugeLinkTensor::try_from_tensor(wrong_shape, lattice),
