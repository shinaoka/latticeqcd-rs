...
## Phase 4 staggered measurements and frontend

The `measurements` crate's optional `fermions` feature exposes the corrected
staggered pion correlator and stochastic chiral condensate. The checked
frontend is intentionally strict and is exercised with:

```bash
cargo run -p latticeqcd -- examples/phase4.toml
```

The independent Phase 4 evidence uses the predeclared `[2,2,2,2]`, beta `5.7`,
mass `0.5`, `Nf=2`, bounds `[0.0004,64]`, step size `0.01`, two MD steps,
four thermalization trajectories, and sixteen interval-one measurements in
four blocks of four. Each configuration uses all three staggered point sources
and the full sink-color Frobenius contraction, plus exactly two canonical Z4
sources. Julia's per-trajectory/source seeds and generated codes, and Rust's
separate predeclared xoshiro update/source states, are recorded in
[`fixtures/fermion_measurements_phase4_ensemble`](fixtures/fermion_measurements_phase4_ensemble).
The two implementations start from separate cold fields and exchange no
configurations or payload results.

The fixture is generated only from a clean external Julia 1.12.5 project and
pinned Gaugefields.jl `9e5719970770f4497405a856315c90bef7f74449`,
LatticeDiracOperators.jl `bdef628184597815ba3e0cddf2536df767e78a02`,
Wilsonloop.jl `e1a617fdedb19b785f89bdeb13c30e53b20743a7`, and
QCDMeasurements.jl `9e04c37bbd68712cf7a749ae5aff10eb6aae4566`:

```bash
export LATTICEQCD_JULIA_PROJECT=/tmp/latticeqcd-phase4-julia-env
export GAUGEFIELDS_JL_DIR=/home/shinaoka/tensor4all/Gaugefields.jl
export LATTICEDIRACOPERATORS_JL_DIR=/home/shinaoka/tensor4all/LatticeDiracOperators.jl
export QCDMEASUREMENTS_JL_DIR=/home/shinaoka/tensor4all/QCDMeasurements.jl
export WILSONLOOP_JL_DIR=/home/shinaoka/tensor4all/Wilsonloop.jl
export JULIA_NUM_THREADS=1
julia --startup-file=no --project="$LATTICEQCD_JULIA_PROJECT" \
  fixtures/generate.jl fermion_measurements_phase4_ensemble
find fixtures/fermion_measurements_phase4_ensemble -type f -print0 \
  | sort -z | xargs -0 sha256sum | sha256sum
```

The two clean generations currently produce complete-tree hash
`d3b0e00fafe54d590b23b3afad6e75214fefaf81a68f5c8c28e13da008c7f657` (the
metadata payload itself is
`b7c5c3ac4d1042de24dcbbdc2bad591fff95e89b48ba304ebe06e9009db8c9c7`). Run the
generator twice and require identical complete-tree hashes. The Julia
oracle never calls the pinned high-level pion reconstruction or
`Z4_distribution_fermi!`: Issue #27's `pi/4` source is replaced by explicit
canonical phases, Issue #29's source-diagonal sink duplication is replaced by
the corrected contraction, and the fixed schedule avoids the pinned
LatticeQCD.jl v1.3.7 (`c09de20aae10f28f6a9c7e84e7711fce94d50915`) parser/scheduler candidates in Issue #30. Rust repeats an
independent stream in `cargo test -p measurements --all-features --test
phase4_ensemble -- --nocapture`; it reports every trajectory's acceptance and
delta-H, every measured pion timeslice/chiral value, block means, and the
normalized differences. The gate for each pion timeslice and the chiral scalar
is `abs(mean_rust - mean_julia) <= 6*sqrt(se_rust^2 + se_julia^2)`, where the
standard errors are computed from the four block means; zero combined error
requires an exact zero difference rather than a division.

The generated Julia statistics are evidence for the fixed stream, not a claim
of bitwise ensemble agreement. Four consecutive blocks give a deliberately
small, autocorrelated consistency estimate rather than a calibrated physics
precision or significance claim. In the recorded run, all 16 Julia measurements
were accepted; the Rust run independently accepted all 16 measured updates
(and all four thermalization updates). Rust/Julia normalized differences were
`0.3856782` and `2.609377` for pion timeslices `t=0,1`, and `0.06207383` for
the chiral scalar, all below six. The metadata records the four Julia blocks
and all Julia delta-H values; the Rust test independently computes and prints
the corresponding Rust blocks and trajectory diagnostics. The recorded Rust
stream states are predeclared constants consumed only by that test, not results
or configurations generated by Julia.
